...
import com.example.tarota5scores.ui.theme.TarotA5ScoressTheme
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /*
        val file = File(filesDir, "historique.json")
        Log.d("HISTO", "path=${file.absolutePath} exists=${file.exists()} size=${file.length()}")
        if (file.exists()) {
            try {
                val content = file.readText()
                Log.d("HISTO", "contenu historique.json:\n$content")
            } catch (e: Exception) {
                Log.e("HISTO", "Erreur lecture historique.json", e)
            }
        }
         */
    }
...
sealed class Screen {
    ...
    object StatistiquesPartie : Screen() // AJOUT de cette ligne
}
...
val ScreenSaver = Saver<Screen, String>(
    save = { screen ->
        when (screen) {
            ...
            is Screen.StatistiquesPartie -> "StatistiquesPartie" // AJOUT de cette ligne
        }
    },
    restore = { name ->
        when (name) {
            ...
            "StatistiquesPartie" -> Screen.StatistiquesPartie // AJOUT de cette ligne
        ...
        AlertDialog(
            ...
            confirmButton = {
                ...
                TextButton(onClick = {
                    ...
                    scope.launch {
                        kotlinx.coroutines.delay(50) // Court délai pour forcer la recomposition
                        currentScreen = Screen.Donne
                    }
                }) {
                    Text("Éditer")
